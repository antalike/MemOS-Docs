---
title: MemOS を使用して本番レベルの知識質問応答システムを構築する
---
## **前書き**

特定分野の知識質問応答（QA）システムを構築する際、業界は一般的な課題に直面しています。大規模言語モデル（LLM）は知識が広範である一方、専門分野における正確性と信頼性にはなお不足があります。また、従来の検索拡張生成（RAG）手法は外部知識を導入できるものの、文書の離散性と深い論理的関連の欠如に制約され、複雑な推論型質問応答への対応が困難です。

本章の目的は、皆さんにMemOSに基づいてこの問題をどのように解決するかを示し、完全な本番レベルの知識拡張ソリューションdemoを提案し実践することです。私たちの中核目標は、重要な主張を証明し実現することです。すなわち、構造化された知識体系を通じて、入念に強化された小型モデルは、強化されていない大規模モデルを専門能力で上回ることができる、ということです。

この目標を実現するため、私たちはMemCubeという動的知識システムを設計・構築しました。その構築フローは、厳密なエンジニアリング方法論に従います。

**暗黙知の発掘と構造化**：まず、特定分野に関する大規模言語モデル（LLM）内部の暗黙知を体系的に発掘し、"概念グラフ反復拡張"の方法によって、それを大規模で高カバレッジの明示的な概念関係グラフへと変換します。

**構造化知識対の生成**：続いて、この概念グラフを指針として、再びLLMを利用し、複雑な臨床ロジックを含む大量の高品質な質問応答（QA）知識対を生成し、知識ベースの中核コンテンツとします。

**知識ベースの構築とデプロイ**：最後に、これらのQA知識対を整理してグラフデータベース（Neo4j）にロードし、MemOSシステムが効率的に検索できる動的知識ベース（MemCube）を形成して、小型モデルの分野能力を強化します。

本章では、心血管医学分野の事例を通じて、ゼロからMemCubeを構築する全プロセスを完全に示し、定量評価を通じて、このシステムがモデルの専門質問応答能力を向上させるうえでの顕著な効果を検証します。これにより、実際の業務で低コスト・高精度・説明可能なAI知識サービスを実現するための再利用可能な標準フローを提供します。

---

## **章の導入：動的知識体系構築の青写真**

本章の中核研究目標は、MemOS 知識ベース（すなわち MemCube）を体系的に構築することで、比較的小さなパラメータ規模（7Bクラスなど）のモデルが、専門分野の質問応答性能において、大規模モデル（32B+クラスなど）に匹敵し、さらにはそれを上回る性能水準に到達できることを検証することです。

本章では、完全な知識工学の実践へとご案内します。私たちの最終目標は、心血管病学分野専用の知能質問応答システムを構築することです。この目標を達成するために、各段階が前段階の成果物を基盤とする、明確で段階的な道筋に従います。

本章全体の流れは次のとおりです。

### **第一段階：分野知識の基礎構造を構築する——概念グラフ拡張**

これはすべての作業の基盤です。高品質な知識ベースは、包括的かつ構造化された分野概念ネットワークから始まり、このネットワークは当該分野の暗黙知を明示的に表現したものです。

**目標**：心血管病学分野の中核概念とその相互関係を広範にカバーできる大規模グラフを作成すること。

**シード概念の取得**：専門的な医学データセットから心血管分野の資料を選別し、LLMを用いてまず高品質な"シード概念"のバッチを抽出し、グラフの起点とします。

**反復拡張**：シード概念を基盤として、複数ラウンドの自動化フローを通じて、LLMにグラフ内の既存概念に基づく連想を行わせ、新たな関連概念を継続的に生成し、接続を確立します。

**収束と評価**：厳格な収束制御メカニズム（類似ノードの統合、新概念増加率の監視など）を導入し、十分な知識カバレッジに達した後にグラフの成長を停止することを保証します。最終的に、外部知識ソースのキーワードとの比較を通じて、グラフの完全性を定量評価します。

### **第二段階：適用可能な知識コンテンツを生成する——グラフに基づくQA対生成**

概念グラフという基礎構造が得られたら、次にAIが直接理解して利用できる具体的知識、すなわち質問応答対をそこへ充填する必要があります。

**目標**：グラフ内の抽象的な概念と関係を、大量の具体的で臨床ロジックを含むQA対へと変換すること。

**単一概念知識生成**：グラフ内の各中核概念ノードを走査し、LLMを用いて各概念ごとに独立した、深みのある臨床問題と回答を生成します。

**関連知識生成**：グラフ内で接続が存在する"概念対"については、両者の内在的ロジックを反映できる、より複雑な関連性問題をLLMに生成させます。

### **第三段階：知識ベースを組み立ててデプロイする——MemCubeの構築とマウント**

離散的なQAデータは、高効率なシステムとして整理される必要があります。

**目標**：生成されたすべてのQAデータを構造化し、グラフデータベースにロードして、MemOSがいつでも呼び出せる知識ベースを形成すること。

**フロー**：

**データ形式化**：すべてのQA対を標準JSON形式に統一し、検索に用いる質問テキストに対してベクトル埋め込み（Embedding）を生成します。

**グラフデータベースへのインポート**：スクリプトを作成し、形式化後のノード（概念、QA）とエッジ（関係）データを一括かつ高効率でNeo4jデータベースにインポートします。

**MemOSマウント**：最後に、簡単な設定を通じてMemOSシステムをこのNeo4jデータベースに向け、私たちの心血管MemCubeを正式に有効化します。

### **第四段階：最終成果を検証する——システム評価**

構築完了後、その価値を客観的データで証明する必要があります。

**目標**：MemCubeを装備した比較的小さな大規模モデルが、強化されていない大規模モデルと比較して、専門質問応答能力で超越を実現したかどうかを定量評価すること。

**フロー**：独立した評価セットを構築し、自動化スクリプトを通じて、異なる構成下のモデルに対して"同一問題競技"を行い、より強いモデルを審判として採点し、最終的に勝率とスコアで私たちのシステムの実際の効果を示します。

以上の四段階を通じて、抽象的な業務要件が、厳密なエンジニアリング手法によってどのように一歩ずつ、最終的に強力で評価可能なAI知識システムとして実装されるのかを、明確にご覧いただけます。
------------------------------------------------------------------------------------------------------------------------------
...
特定分野の非構造化知識を、高品質で構造化されたシード概念集合へと変換すること。これは知能MemCubeを構築するための礎石です。

### **中核理念**

専門的な質問応答シナリオでは、大規模言語モデル（LLM）の学習データにはすでに膨大な分野知識が暗黙的に含まれています。課題は、これらの知識をどのように体系的に"抽出"し"整理"するかにあり、それによって分野専用の小型MemCubeが大規模モデルに匹敵する知識能力を備えられるようにすることです。

LLMに直接「心血管分野のすべての知識を提供してください」と質問するのは非効率であり、実行不可能です。したがって、まず一連の正確な\*\*「概念アンカー」\*\*を確立し、それを基礎として、LLM内部の当該分野に関する知識グラフを体系的に描き出さなければなりません。分野知識のカバレッジは直接定量化しにくいものの、分野内の中核概念およびキーワードの完全性を通じて間接的に測定できます。このステップの最終目的は、分野概念をできる限り包括的に捕捉し、後続の知識抽出に構造化された支援を提供することです。

---

### **Step 1: シード概念の取得**

グラフの構築は、高品質な"シード概念"のバッチを取得することから始まります。これらの初期概念は、グラフの反復拡張の出発点であり、その品質は、モデルが分野知識フレームワークを構築する速度と効率に直接影響します。

シード概念の専門性とカバレッジを確保するため、本実験では公開医学専門家試験データセット `medxpert` を初期データソースとして採用します。このデータセットには明確な医学分野分類が含まれており、心血管分野に関連する知識を正確に選別するのに便利です。

心より感謝します：MedXpert公開Benchmark

データダウンロード先：https://raw.githubusercontent.com/TsinghuaC3I/MedXpertQA/refs/heads/main/eval/data/medxpertqa/input/medxpertqa_text_input.jsonl

以下のコードは、データのロードと選別のプロセスを示しています。
...
```python
# api url; api keyはお客様専用の設定です
# 具体的に使用するllmモデルについて：
# 1. seed conceptsを要約・整理する段階では、強いモデルMEDXPERT_THINKER=o3を使用して、既存のmedxpert心血管分野100+問題を拡張し、その問題分析の思考過程を抽出します。私たちの目的は、o3に各問題へ正しく回答させることではなく、各心血管問題を思考・分析した後、その思考過程から心血管シード概念を要約させることにあります。
# 2. o3の思考過程を抽出した後、MEDXPERT_THINK_EXTRACTOR=gpt-4oを用いて、その思考過程から心血管概念名詞をシード概念として抽出します
# 上記2つの段階の目的は、シード概念ライブラリを構築することです。お客様専用の分野文書ライブラリをお持ちであれば、この段階をスキップし、関心のあるシード概念をお客様の分野文書から直接抽出できます。
# 3. シード概念ライブラリの構築完了後、分野概念グラフの拡張を行います。この段階では、私たちの実験結果とお客様のコスト予算に基づいて、適切にモデルを選択することを推奨します。私たちの試験段階では、CONCEPT_GRAPH_EXTENDER=gpt-4ominiを参考としています。
# 4. 分野概念グラフの構築完了後、(a)各概念に対する個別のqa生成 (b)意味のある各概念対に対するqa生成を行います。qa生成はモデル能力に依存するため、強いモデルの使用を推奨します。私たちの試験段階では、QA_SYNTHESIZER=gpt-4oを参考としています。
# すべてのembeddingモデルについて、英語embeddingのオープンソースモデルEMBEDDING_MODEL=nomic-ai/nomic-embed-text-v1.5を適用します
...
本章で紹介した完全な構築フローと最終的な効果を自ら体験したいですか？当社のデモプロジェクトにアクセスできます：

**👉 [Cardio MemCube Demo - Hugging Face](https://huggingface.co/datasets/MemCube/cardio-memcube-demo)**

このデモプロジェクトは以下を提供します：
- ✅ **完全なインストールガイド**：ワンクリックで心血管 MemCube システムをデプロイ
- ✅ **実行可能なコード例**：知識Q&A機能を直接体験
- ✅ **詳細な技術ドキュメント**：構築方法論とベストプラクティスを理解
- ✅ **多言語モデルサポート**：異なる AI モデルバックエンドを柔軟に構成

### ⚠️ 重要な説明

- **🏥 医学免責事項**：このデモは技術展示および教育目的のみに使用され、医学的診断または治療の根拠として使用すべきではありません
- **🌐 言語サポート**：現在のバージョンでは英語向けに最適化された埋め込みモデルを使用しており、中国語でのクエリには翻訳または多言語埋め込みモデルへの切り替えが必要です
- **🔧 技術フレームワーク**：これはあらゆる専門分野に適用可能な技術リファレンス実装です

このデモシステムを実際に体験することで、本章の理論的方法を実際の本番環境レベルのアプリケーションへどのように変換するかをよりよく理解し、自身の分野の MemCube システムを構築するための貴重な経験を蓄積できるでしょう。
