...
description: 动手实战！我们将带您使用 **SimpleStructMemReader** 从对话中提取记忆，并把它存进 **TreeTextMemory** 进行管理与检索。
---
...
- **探索 [TreeTextMemory](/cn/open_source/modules/memories/textual/tree_textual_memory)：** 构建基于图的层级记忆。
- **添加 [Activation Memory](/cn/open_source/modules/memories/kv_cache_memory)：** 缓存键值状态，加速推理。
- **深入学习：** 查看 [API Reference](/cn/api-reference/search-memories) 和 [Examples](/cn/open_source/getting_started/examples) 了解高级工作流程。

接下来，您可以去看看更高级的玩法：
- **[MemReader](/cn/open_source/modules/mem_reader)**：其实它还能读图片和 PDF。
- **[MemFeedback](/cn/open_source/modules/mem_feedback)**：如果有记忆记错了，怎么让 AI 自动修正？
- **[MemCube](/cn/open_source/modules/mem_cube)**：怎么把各种记忆能力打包在一起，做一个真正的全能大脑。
