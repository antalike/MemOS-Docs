...
title: MemOS 示例
icon: ri:code-line
description: 恭喜你——你已经掌握了快速入门并构建了第一个可用的记忆！现在是时候通过结合不同的记忆类型和功能，看看 MemOS 可以实现多大的可能性。使用这些精选示例来激发你自己的智能体、聊天机器人或知识系统的灵感。
---

<Cards>

<Card title={"最简Pipeline"} icon={<Icon name="ri:play-line" />} href="/cn/open_source/getting_started/examples#示例-1最简pipeline">

最小的可用Pipeline  — 添加、搜索明文记忆。

</Card>

<Card title={"多信息源的添加与检索"} icon={<Icon name="ri:tree-line" />} href="/cn/open_source/getting_started/examples#示例-2多信息源记忆的添加与检索">

添加文本、图片、文件、工具调用的多信息源messages到记忆，并能够检索它们。

</Card>

<Card title={"多Cube添加和检索"} icon={<Icon name="ri:apps-line" />} href="/cn/open_source/getting_started/examples#示例-3多cube添加和检索">

添加不同记忆到不同的Cube，在检索时同时召回它们。

</Card>

<Card title={"仅 KVCacheMemory"} icon={<Icon name="ri:database-2-line" />} href="/cn/open_source/getting_started/examples#示例-4仅-kvcachememory">

使用短期 KV cache加速会话，实现快速上下文注入。

</Card>

<Card title={"记忆调度"} icon={<Icon name="ri:calendar-check-line" />} href="/cn/open_source/getting_started/examples#示例-5多忆调度">
