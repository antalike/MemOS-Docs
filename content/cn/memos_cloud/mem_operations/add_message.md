...
| 元信息 | `info` | 
自定义元信息字段，用于补充当前用户的对话消息，并在后续的记忆检索中作为过滤条件使用。
	•	info 支持传入任意自定义的键值对，用于描述业务类型、业务标识、使用场景、用户状态等上下文信息
	•	所有通过 info 传入的字段，都会随消息一并存储，并可在后续记忆检索中使用
	•	当前系统对以下字段做了性能优化支持，使用这些字段进行检索时效率更高：
	•	business_type（业务类型）
	•	biz_id（业务唯一标识）
	•	scene（业务或对话场景）
	•	custom_status（自定义状态）

说明：是否使用上述字段并非强制要求，其他自定义字段同样可以正常存储和检索。 |
