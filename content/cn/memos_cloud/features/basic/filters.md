...
| 字段名 | 数据类型 | 操作符 | 示例 | 备注 | 
| --- | --- | --- | --- | --- |
| business\_type | str | `=` | `{"business_type":"购物"}` | 已添加数据库索引，查询速度更快 |
| biz\_id | str | `=` | `{"biz_id":"order_123456"}` | 已添加数据库索引，查询速度更快 |
| scene | str | `=` | `{"scene": "支付"}` | 已添加数据库索引，查询速度更快 |
| custom\_status | str | `=` | `{"custom_status": "VIP3"}` | 可自行定义其他字段名 |
