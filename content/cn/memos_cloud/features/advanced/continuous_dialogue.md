...
接口输入上限： 10,000 tokens。

接口输出上限：

*   检索记忆条数——事实记忆25条；偏好记忆25条；工具记忆25条。

*   回答输出Token数——10,000 tokens。
...
&nbsp;有关 API 字段、格式等信息的完整列表，详见++Chat 接口文档++。
::
...
| 召回偏好记忆   | `include_preference`<br>`preference_limit_number` | 偏好记忆是 MemOS 基于用户历史消息分析生成的用户偏好信息。<br>开启后，可在检索结果中召回用户偏好记忆，“更懂用户”。 |
...
::note{icon="websymbol:chat"}
&nbsp;如果您想让模型在回答时更好地参考记忆，构建`system_prompt`时可以参考当前 MemOS 默认的指令。如下所示，当前`<memories>`
