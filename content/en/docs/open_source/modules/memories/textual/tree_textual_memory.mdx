...
icon: ri:tree-line
description: |
  Let’s build your first **graph-based, tree-structured memory** in MemOS! <br> **TreeTextMemory** helps you organize, link, and retrieve memories with rich context and explainability. <br> [Neo4j](/open_source/modules/memories/textual/neo4j_graph_db) is the current backend, with support for additional graph stores planned in the future.
---
...
- **Know more about [Neo4j](/open_source/modules/memories/textual/neo4j_graph_db):** TreeTextMemory is powered by a graph database backend.
