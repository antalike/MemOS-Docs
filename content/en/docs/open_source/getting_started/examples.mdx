...
icon: ri:code-line
description: Congratulations - you've mastered the Quick Start and built your first working memory! Now it's time to see how far you can take MemOS by combining different memory types and features. Use these curated examples to inspire your own agents, chatbots, or knowledge systems.
---
...
<Card title={"Adding and retrieving multiple information sources"} icon={<Icon name="ri:tree-line" />} href="/open_source/getting_started/examples#example-2-multi-modal">

Adding multi-source messages—including text, images, files, and tool calls—into memory and enabling their retrieval.

</Card>

<Card title={"Multi-Cube addition and retrieval"} icon={<Icon name="ri:apps-line" />} href="/open_source/getting_started/examples#example-3-multi-cube">

Add different memories to different Cubes and retrieve them simultaneously during a search.

</Card>

<Card title={"KVCacheMemory Only"} icon={<Icon name="ri:database-2-line" />} href="/open_source/getting_started/examples#example-4-kvcachememory-only">

Speed up sessions with short-term KV cache for fast context injection.

</Card>

<Card title={"Hybrid TreeText + KVCache"} icon={<Icon name="hugeicons:share-07" />} href="/open_source/getting_started/examples#example-5-hybrid">

Combine explainable graph memory with fast KV caching in a single MemCube.

</Card>

<Card title={"Multi-Memory Scheduling"} icon={<Icon name="ri:calendar-check-line" />} href="/open_source/getting_started/examples#example-6-multi-memory-scheduling">
